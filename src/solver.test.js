...
//   expect(solveNonogram(getRowHints(data), getColumnHints(data)).solved).toBe(true)
...
//   expect(solveNonogram(getRowHints(board), getColumnHints(board)).solved).toBe(true)
// })
...
test('seed many random puzzles', () => {
  ...
  let result = Array.from({length: 30}, (_, i) => {
    ...
    let isSolved = solveNonogram(getRowHints(data), getColumnHints(data)).solved
  ...
  console.log('solved', result.filter((value) => value).length)
  console.log('not solved', result.filter((value) => !value).length)
